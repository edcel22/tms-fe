<template>
  <div class="page" v-if="loaded">
    <div class="actions">
			<nuxt-link :to="`/tasks/create`" class="primary_button pointer">
      ...
      <table class="table">
        ...
        <tbody v-if="tasks && tasks.length > 0">
          <tr v-for="(task, key) in tasks" :key="key">
            <td>
              <nuxt-link :to="`/tasks/${task.id}`" class="clickable">{{ task.title }}</nuxt-link>
            </td>
            <td>{{ task.description }}</td>
            <td>{{ $moment(task.due_date, 'DD/MM/YYYY').format("MMM Do YY") }}</td>
            <td>{{ completionStatus(task.completion_status) }}</td>
            <td class="buttons">
              <div class="wrapper">
                  <div class="item ml delete pointer tooltip" @click="toggleConfirmation(task)">
                      <span>
  ...
  export default {
    ...
    methods: {
      toggleConfirmation (data) {
          setTimeout( () => {
              this.api = `/tasks/task/${data.id}/delete`
