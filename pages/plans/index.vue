<template>
  <div class="page" v-if="loaded">
    <div class="actions">
			<nuxt-link :to="`/plans/create`" class="primary_button pointer">
    ...
    <div class="box">
      <table class="table">
        ...
        <tbody v-if="plans && plans.length > 0">
          <tr v-for="(plan, key) in plans" :key="key">
            <td>
              <nuxt-link :to="`/plans/${plan.id}`" class="clickable">{{ plan.name }}</nuxt-link>
            </td>
            <td>{{ (plan.service) ? plan.service.name : '' }}</td>
            <td>{{ plan.cost_to_platform }}</td>
            <td>{{ plan.social_owl_margin }}</td>
            <td>{{ plan.wholesale_price }}</td>
            <td>{{ activeStatus(plan.status_id) }}</td>
            <td class="buttons">
              <div class="wrapper">
                  <div class="item ml delete pointer tooltip" @click="toggleConfirmation(plan)">
                      <span>
