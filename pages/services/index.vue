<template>
  <div class="page" v-if="loaded">
    <div class="actions">
			<nuxt-link :to="`/services/create`" class="primary_button pointer">
      ...
      <table class="table">
        ...
        <tbody v-if="services && services.length > 0">
          <tr v-for="(service, key) in services" :key="key">
            <td>
              <nuxt-link :to="`/services/${service.id}`" class="clickable">{{ service.name }}</nuxt-link>
            </td>
            <td>{{ activeStatus(service.status_id) }}</td>
            <td class="buttons">
              <div class="wrapper">
                  <div class="item ml delete pointer tooltip" @click="toggleConfirmation(service)">
                      <span>
  ...
  export default {
    ...
    methods: {
      toggleConfirmation (data) {
          setTimeout( () => {
              this.api = `/services/${data.id}/delete`
