<template>
  <div class="page" v-if="loaded">
    <div class="actions">
			<nuxt-link :to="`/customers/create`" class="primary_button pointer">
      ...
      <table class="table">
        ...
        <tbody v-if="customers && customers.length > 0">
          <tr v-for="(customer, key) in customers" :key="key">
            <td>
              <nuxt-link :to="`/customers/${customer.id}`" class="clickable">{{ customer.name }}</nuxt-link>
            </td>
            <td>{{ customer.email }}</td>
            <td>{{ customer.user_detail.contact_number }}</td>
            <td>{{ customer.user_detail.adress }} {{ customer.user_detail.city }} {{ customer.user_detail.zip }}</td>
            <td>{{ activeStatus(customer.is_active) }}</td>
            <td class="buttons">
              <div class="wrapper">
                  <div class="item ml delete pointer tooltip" @click="toggleConfirmation(customer)">
                      <span>
  ...
  export default {
    ...
    methods: {
      toggleConfirmation (data) {
          setTimeout( () => {
              this.api = `/customers/${data.id}/delete`
